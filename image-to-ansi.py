...
def print_help():
    ...
    print("Source: https://github.com/ickyicky/image-to-ansi")
...
def fit(im):
    ...
    max_h, max_w = int(max_h), int(int(max_w) / 2)

    if im.size[0] <= max_w and im.size[1] <= max_h:
        return im

    wscale = max_w / float(im.size[0])
    hscale = max_h / float(im.size[1])
    scale = min(hscale, wscale)

    wsize = int(float(im.size[0] * scale))
    hsize = int(float(im.size[1] * scale))
